...
class DecisionTreeRegressor:
    ...
    def feature_importances_(self):
        """
        Compute feature importances based on the sum of squared error reductions.
        ...
        if not hasattr(self, 'feature_importances'):
            # Calculate based on your tree structure
            # This is a simplified version - actual implementation depends on your tree
            self.feature_importances = np.zeros(self.n_features_)
            self._calculate_importances(self.tree_)
            # Normalize
    ...
    def _calculate_importances(self, node):
        ...
        if node.is_leaf:
            return
        self.feature_importances[node.feature_index] += node.impurity_reduction
        self._calculate_importances(node.left_child)
        self._calculate_importances(node.right_child)